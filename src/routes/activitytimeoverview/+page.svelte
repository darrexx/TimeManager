<script lang="ts">
	...
	onMount(async () => {
		...
		activityTimes = await invoke('get_activity_times');
	});
</script>
...
<div class="bg-white dark:bg-gray-900 text-gray-600 dark:text-gray-400 py-2 min-h-screen">
	<div class="grid grid-cols-5 justify-items-center items-center">
		...
		<Heading
			...
			customSize="text-4xl font-extrabold  md:text-5xl lg:text-5xl">Activity Time Overview</Heading
		>
		<div />
	</div>
	...
	<ActivityTimeTable
		...
		activityTimes={activityTimes.map(createActivityTime)}
	/>
</div>
